...
async def stats(ctx):
    ...
    embed.add_field(name='Bot Developer:',
                    value="https://github.com/likweitan/discord-bot")
...
async def on_message(message):
    ...
    if "http" in message.content:
        ...
        embed.add_field(name='Reference Link',
                        value=response.json()['permalink'])
        if response.json()['positives'] / response.json()['total'] > 0.8:
            ...
            await message.add_reaction("👍")
        embed.set_footer(
            text=f"Last scanned on { response.json()['scan_date'] }")
        embed.set_author(name=bot.user.name, icon_url=bot.user.avatar_url)
        # embed.set_thumbnail(url='https://avatars.githubusercontent.com/u/11171910')
...
async def corona(ctx, arg):
    ...
    embed.add_field(name="undefined", value="Total new cases today: " +
                    str(total_new_cases), inline=False)
    await ctx.send(embed=embed)
...
async def profile(ctx, arg):
    if arg == 'huanming':
        ...
        embed = discord.Embed(title=f"{ ctx.author.discriminator } Stats", description='\uFEFF',
                              colour=ctx.author.colour, timestamp=ctx.message.created_at)
...
async def wiki(ctx, arg):
    ...
    try:
        ...
        answer = '"' + str(arg) + \
            '" does not match any pages. Try another query!'
